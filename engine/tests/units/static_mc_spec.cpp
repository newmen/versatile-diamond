...
template <class MC>
UVector ubiquitousPack(MC *mc, Atom *atom)
{
    return {
        new UEvent1<MC>(mc, atom),
        new UEvent2<MC>(mc, atom)
    };
}

template <class MC>
UVector makeUbiquitous(MC *mc, Atom *atom1, Atom *atom2, Atom *atom3)
{
    return concat({
...
template <class MC, template <class> class TEvent>
SVector typicalPack(MC *mc, ushort n)
{
    SVector result;
    for (ushort i = 0; i < n; ++i) result.push_back(new TEvent<MC>(mc));
    return result;
}

template <class MC>
SVector makeTypical(MC *mc, ushort n)
{
    ushort i = 0;
    return concat({
        typicalPack<MC, TEvent3>(mc, n - tRates[i++]),
        typicalPack<MC, TEvent5>(mc, n - tRates[i++]),
        typicalPack<MC, TEvent7>(mc, n - tRates[i++]),
        typicalPack<MC, TEvent11>(mc, n - tRates[i++]),
        typicalPack<MC, TEvent13>(mc, n - tRates[i++]),
        typicalPack<MC, TEvent17>(mc, n - tRates[i++]),
        typicalPack<MC, TEvent19>(mc, n - tRates[i++])
    });
}
...
template <class MC>
void assertMC(MC *mc, bool hasTotalTime, double totalRate)
{
...
{
    check<DynamicMC<FakeMCData, FAKE_TS_NUM, FAKE_US_NUM>>();
    check<TreeMC<FakeMCData, FAKE_TS_NUM, FAKE_US_NUM>>();
