...
public:
    template <class R, ushort ATOMS_NUM, ushort OT, ushort ST>
    static void find(SpecificSpec *target, const ushort *indexes, const ushort *types, RelationFunc nLambda);
...
template <ushort RT, ushort SCA_RT, ushort TARGETS_NUM>
template <class R, ushort ATOMS_NUM, ushort OT, ushort ST>
void ManyTypical<RT, SCA_RT, TARGETS_NUM>::find(SpecificSpec *target, const ushort *indexes, const ushort *types, RelationFunc nLambda)
{
    ...
    if ((result && nbrs[0]) || (nbrs[0] && !nbrs[0]->prevIs(OT)))
    {
        FewSpecsReaction<TARGETS_NUM>::template addIfHasNeighbour<R>(target, nbrs[0], OT, ST);
    }
    bool hasAnother = nbrs[1] && nbrs[1]->isVisited();
    if ((result && hasAnother) || (hasAnother && !nbrs[1]->prevIs(OT)))
    {
        FewSpecsReaction<TARGETS_NUM>::template addIfHasNeighbour<R>(target, nbrs[1], OT, ST);
    }
}
