...
PROGRESS_RX = re.compile(r'^\s*(?P<percent>\d+(?:\.\d+)?) %\s+(?P<surf_num>\d+)\s+(?P<amorph_num>\d+)\s+(?P<act_rate>\d+(?:\.\d+)?) %\s+(?P<seconds>\d+(?:\.\d+)?) \(s\)\s+(?P<neg_rate>\d+(?:\.\d+)?(?:e\+\d+)?) \(1/s\)$')
PROGRESS_CLS = ['percent', 'surf_num', 'amorph_num', 'act_rate', 'seconds', 'neg_rate']

FREQ_STAT_RX = re.compile(r'^\s*(?P<name>(?:\w|\s)+?) ::\s*(?P<quantity>\d+) ::\s+(?P<freq>\d+(?:\.\d+)?(?:e\+\d+)?) % :: (?P<rate>\d+(?:\.\d+)?(?:e\+\d+)?)$')
