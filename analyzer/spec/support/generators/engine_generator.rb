  ...
  module Generators
    module Support
      ...
      module EngineGenerator
        ...
        include Organizers::ReactionsOrganizer
      ...
      private
        ...
        def fix_reactants(depts_cache, all_names)
          [:ubiquitous_reactions, :typical_reactions, :lateral_reactions].each do |k|
            depts_cache[k].each do |dr|
              r = dr.reaction
              (r.source + r.products).each do |s|
                next if all_names.include?(s.name)
                if s.is_a?(Concepts::TerminationSpec)
                  all_names << s.name
                  depts_cache[:term_specs] << Organizers::DependentTermination.new(s)
                else
        ...
        def fix_bases(depts_cache, all_names)
          depts_cache[:specific_specs].each do |ds|
            ...
            depts_cache[:base_specs] << Organizers::DependentBaseSpec.new(ds.spec.spec)
          end
        end

        # Provides lambda which checks type of own argument and wraps and store it
        # to passed variables
        def store_reactant_lambda(depts_cache, all_names)
          -> s do
            all_names << s.name
            if s.is_a?(Concepts::SpecificSpec)
              depts_cache[:specific_specs] << Organizers::DependentSpecificSpec.new(s)
            elsif s.is_a?(Concepts::Spec)
              depts_cache[:base_specs] << Organizers::DependentBaseSpec.new(s)
            else
